...
## Step 6 — Migrations (run in SQL Editor, in order)
| File | What it adds |
|------|--------------|
| `migration_runtime_hours.sql` | Machine runtime tracking |
| `migration_step5.sql` / `migration_step6.sql` | Earlier feature migrations |
| `migration_rul_range.sql` | `rul_low` / `rul_high` columns — 80% prediction-interval bounds from the ML server's quantile RUL model. App works without it, but ranges won't be stored. |
