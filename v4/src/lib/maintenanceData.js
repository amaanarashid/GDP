...
// ── Reset machine (RPC) ────────────────────────────────────
export async function resetMachine(machineId, wipeHistory = false, resetRuntime = false) {
  const { error } = await supabase.rpc('reset_machine', {
    p_machine_id: machineId,
    p_wipe_history: wipeHistory,
    p_reset_runtime: resetRuntime,
  })
...
export async function writeRULPredictions(predictions) {
  // predictions: [{ machine_id, component_id, days_remaining, confidence, rul_low, rul_high }]
  const { error } = await supabase.from('rul_predictions').insert(predictions)
  if (error) {
    // rul_low/rul_high need data/migration_rul_range.sql — if it hasn't
    // been run yet, retry without the range columns so nothing breaks.
    if (`${error.message}`.includes('rul_low') || `${error.message}`.includes('rul_high')) {
      const stripped = predictions.map(({ rul_low: _lo, rul_high: _hi, ...rest }) => rest)
      const { error: e2 } = await supabase.from('rul_predictions').insert(stripped)
      if (e2) console.error('writeRUL', e2)
      return
    }
    console.error('writeRUL', error)
  }
}

// ── RUL prediction history (for predicted-vs-actual view) ──
export async function getRulPredictionHistory(machineId, limit = 400) {
  const [{ data: preds, error: pErr }, { data: events, error: aErr }] = await Promise.all([
    supabase
      .from('rul_predictions')
      .select('component_id, days_remaining, confidence, timestamp')
  ...
  if (aErr) throw aErr
  return {
    predictions: (preds || []).reverse(),   // oldest → newest
    failureEvents: (events || []).reverse(),
  }
}
