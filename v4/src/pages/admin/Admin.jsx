...
import { softDeleteMachine, restoreMachine, deleteMachine } from '../../lib/adminData'
...
import { Plus, Pencil, QrCode, Archive, RotateCcw, Trash2 } from 'lucide-react'
...
export default function Admin() {
  ...
  async function handleDelete(machine) {
    if (machine.status === 'offline') {
      ...
      if (!window.confirm(`Archive "${machine.name}"? It will be hidden from the dashboard (can be restored here).`)) return
      await softDeleteMachine(machine.id)
    }
    load()
  }

  async function handleHardDelete(machine) {
    if (!window.confirm(`Permanently DELETE "${machine.name}"?\n\nThis removes the machine and ALL its data (readings, alerts, predictions, maintenance history). This cannot be undone.`)) return
    try {
                ...
                {machines.length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center text-gray-500 py-10">
                      No machines yet — click <span className="text-gray-700">Add machine</span> to create one.
                    </td>
                  </tr>
                )}
                {machines.map(m => (
                  <tr key={m.id} className={m.status === 'offline' ? 'opacity-50' : ''}>
                    ...
                    <td className="text-gray-600">{machineTypeLabel(m.type)}</td>
                    <td className="text-gray-600">{m.components?.length || 0}</td>
                    <td className="text-gray-600">{fmtHours(m.runtime_hours)}</td>
                    <td><span className={statusBadge(m.status)}>{m.status}</span></td>
                    <td>
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => setQr(m)} className="icon-btn" title="QR code">
                          <QrCode className="w-4 h-4" />
                        </button>
                        <button onClick={() => setEdit(m)} className="icon-btn" title="Edit">
                        ...
                        <button onClick={() => handleHardDelete(m)}
