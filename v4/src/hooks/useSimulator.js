...
const TICK_MS = 2000        // simulation tick interval (2s)
...
export function useSimulator({ machine, components, sensors, onUpdate }) {
  ...
  // Seed runtime from machine — intentionally only on machine CHANGE:
  // re-running on every runtime_hours update would fight incrementRuntime.
  useEffect(() => {
  ...
  const tick = useCallback(async () => {
    ...
    const updatedComponents = components.map(c => {
      ...
      const delta = healthDelta(compSensors, hasFault, runtimeRef.current ?? 0, c.id)
