  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="card w-full max-w-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Add new machine</h2>

        {/* Mode toggle */}
        <div className="flex gap-2 mb-4">
          <button onClick={() => setMode('preset')}
            className={`flex-1 py-2 rounded-lg text-sm font-medium border ${mode === 'preset' ? 'bg-indigo-50 border-indigo-600 text-indigo-600' : 'border-gray-200 text-gray-600'}`}>
            From preset
          </button>
          <button onClick={() => setMode('custom')}
            className={`flex-1 py-2 rounded-lg text-sm font-medium border ${mode === 'custom' ? 'bg-indigo-50 border-indigo-600 text-indigo-600' : 'border-gray-200 text-gray-600'}`}>
            Custom build
          </button>
        </div>
        ...
        {mode === 'preset' ? (
          ...
          <div className="mb-4 space-y-3">
            ...
            {components.map((comp, ci) => (
              <div key={ci} className="border border-gray-200 rounded-lg p-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button onClick={() => removeComponent(ci)} className="text-gray-500 hover:text-red-600 p-1">
                ...
                <div className="space-y-2 pl-3 border-l border-gray-200">
                  {comp.sensors.map((s, si) => (
                    <div key={si} className="grid grid-cols-12 gap-1.5 items-center">
                      ...
                      <button onClick={() => removeSensor(ci, si)} className="col-span-1 text-gray-400 hover:text-red-600">
                        <X className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  ))}
                  <button onClick={() => addSensor(ci)} className="text-xs text-indigo-600 hover:text-indigo-700 flex items-center gap-1">
