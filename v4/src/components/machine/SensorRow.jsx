...
export default function SensorRow({ sensor }) {
  ...
  const dotColor = severity === 'critical' ? 'bg-red-500'
    : severity === 'warning' ? 'bg-yellow-500' : 'bg-green-500'
  const valColor = severity === 'critical' ? 'text-red-600'
    : severity === 'warning' ? 'text-yellow-600' : 'text-gray-800'

  return (
    <div className="flex items-center gap-3 py-2">
      <span className={`w-2 h-2 rounded-full shrink-0 ${dotColor}`} />
      <span className="text-sm text-gray-600 w-40 shrink-0 truncate">{sensor.name}</span>
