  ...
  return (
    <g>
      ...
      <text x={x + w / 2} y={y + hgt / 2 - 4} textAnchor="middle"
        fontSize="10" fill="#374151" fontWeight="500">{label}</text>
  ...
  return (
    <svg viewBox="0 0 420 200" className="w-full">
      ...
      <g>
        ...
        <text x="160" y="186" textAnchor="middle" fontSize="10" fill="#374151">Fan {h(components,'Cooling Fan') != null ? Math.round(h(components,'Cooling Fan'))+'%' : ''}</text>
      </g>
      {/* pipes */}
...
// ── GENERIC (any components — used for real/custom machines)─
function GenericTwin({ components, running }) {
  ...
  return (
    <svg viewBox="0 0 420 200" className="w-full">
      {items.map((c, i) => {
        ...
        const y = 20 + row * 80
        const score = parseFloat(c.health_score)
        const fill = healthFill(score)
        const stroke = healthStroke(score)
        ...
        return (
          <g key={c.id}>
            <rect x={x} y={y} width={cellW - 14} height={60} rx={6}
              fill={fill} fillOpacity="0.18" stroke={stroke} strokeWidth="1.5"
              className={critical && running ? 'health-twin-pulse' : ''} />
            <text x={x + (cellW - 14) / 2} y={y + 26} textAnchor="middle" fontSize="10" fill="#374151" fontWeight="500">{short}</text>
            <text x={x + (cellW - 14) / 2} y={y + 44} textAnchor="middle" fontSize="13" fill={fill} fontWeight="600">
              {score != null ? `${Math.round(score)}%` : '—'}
            </text>
          </g>
        )
...
export default function DigitalTwin({ type, components, running = false }) {
  ...
  // Real/custom machines whose components don't match the standard presets
  // render with the generic grid twin.
  const names = (components || []).map(c => c.name)
  const isStandardAir = names.includes('Compressor Pump') || names.includes('Cooling Fan')
  const isMetroOrCustom = type === 'custom'
    || names.includes('Compressor Unit') || names.includes('Pneumatic Panel') || names.includes('Air Dryer')

  if (isMetroOrCustom) return <GenericTwin {...props} />

  switch (type) {
    case 'conveyor_drive':  return <ConveyorTwin {...props} />
    case 'hydraulic_press': return <HydraulicTwin {...props} />
    case 'air_compressor':
      return isStandardAir ? <AirCompressorTwin {...props} /> : <GenericTwin {...props} />
    default:
      return <GenericTwin {...props} />
  }
}
